...
## הכרעות על 4 ההתנגשויות (מ-Claude Code)

**1. התנגשות שמות class (חוסם) — נפתר.** כל המחלקות בחבילה שונו לתחילית ייחודית `shx-` (למשל `shx-btn`, `shx-input`, `shx-table`, `shx-th`, `shx-row`). אין יותר חפיפה עם `ui/ui.css` הקיים. אפשר לייבא `@/styles/shifthouse.css` גלובלית — הוא לא נוגע ב-`.sh-*` הקיימות. (גם משתני ה-CSS הם `--shx-*`.)

**2. פילטרים של הסרגל — searchable.** להתיישר עם האיחוד הקודם: הפילטרים ב-`ItemsToolbar` צריכים להשתמש ב-`SearchableCombobox` (התנהגות **2a**) בדיוק כמו שאר הבוחרים, לא ב-dropdown השטוח הפנימי. ה-dropdown שבתבנית הוא fallback ויזואלי בלבד — החלף אותו ב-2a.

**3. שדות הפופאפ — SearchableCombobox (2a).** לא `<select>` נייטיב. ה-`<Select>` שבקובץ הוא placeholder; החלף את כל הבחירות בפופאפ ב-`SearchableCombobox` (2a). ה-wrapper (`Field` + מרווחים) כבר בסגנון Shifthouse.

**4. מסלול תלוי סוג-פריט — נפתר.** `AddItemDialog` עכשיו מרים state החוצה: prop חדש `onFormChange(form)` נורה בכל שינוי שדה. הקונטיינר קורא ל-`form.itemType` ומעביר `routeOptions` מסונן בהתאם (ומרענן `shipmentItems` לפי `form.shipment`). כך "מסלול מסונן לפי סוג פריט" עובד בלי לשבור את היות הקומפוננטה presentational.

```tsx
const [form, setForm] = React.useState<NewItemForm | null>(null);
const routeOptions = React.useMemo(
  () => routes.filter(r => String(r.item_type_id) === form?.itemType).map(r => ({ value: r.route_number, label: `מסלול ${r.route_number}` })),
  [routes, form?.itemType]
);
<AddItemDialog ... routeOptions={routeOptions} onFormChange={setForm} />
```

> על שאלת ה-autocomplete של Claude Code — **להתעלם**. אנחנו כותבים את `SearchableCombobox` מחדש (וריאציה 2a) ושולחים בנפרד; מה שהוא יבחר שם לא משנה.
