...
### 9.4א הבדלים שנפסקו בפועל ברתמת שלב 5 — עם המספרים

**הרצה:** `scripts/dashboard-parity.js`, DB `testingsite` (630 פריטים · 10,202 events · 6,426 intervals · 630 route_run · 607 סגורים · 13 חודשי היסטוריה · 77 משלוחים · 2,458 שורות `item_route_history`).
**שני הצדדים רצו כשרתים נפרדים מול אותו DB:** הישן מ־worktree של `2fb5664` (‏`--old-api-base :3001`), החדש מעץ העבודה (‏`--new http --new-api-base :3000`) — כלומר ה־endpoints שנשלחים באמת, ולא האדפטרים הפנימיים של הרתמה. הרתמה הורחבה לשם כך ב־`--old-api-base` / `--new impl|http` / `--new-api-base`, בשתי משפחות מקרים נוספות, ובמדידת זמנים.
**מטריצה:** 6 חלונות (`today`, `7d`, `30d`, `90d`, `12mo`, `13mo`) × 11 פילטרים (ללא · לקוח · משלוח · סוג פריט · תחנה · סוג תחנה · עובד · מספר סידורי · `status=queue|processing|finished`) × 2 ערכי `scope` = **132 מקרים ל־endpoint × 10 endpoints (1,320 השוואות)**. תוצאה: **0 הפרשי שדה/שורה בלתי מוסברים, 0 שגיאות — PARITY: GREEN.**

#### הפסיקות — הישן שגוי, ה־ledger מתקן

| מטריקה | ישן | חדש | הראיה |
|---|---|---|---|
| `slow_item_*` (כל המסך) | **`[]` תמיד, HTTP 200** | 20 שורות אמיתיות | `items.item_id` הוא `bigint`; `JSON.stringify` זורק `TypeError: Do not know how to serialize a BigInt` ב־`route.ts:96`, וה־catch מחזיר `[]`. קרה ב־**128 מתוך 132** המקרים (בארבעה הנותרים הסט ריק ממילא). המסך הזה מעולם לא הציג שורה |
| `kpi_items_currently_in_queue` | 15 | **19** | הישן סופר `current_status = 2` בלבד; לוח התחנות **באותו קובץ** סופר `IN (2,4)`. החדש הוא `ms.is_waiting` = `queued`+`queued_research` = 15+4. ה־ledger ו־`item_routes` מסכימים מצב־מול־מצב (1→2, 2→15, 4→4, 5→2) — ההפרש הוא הגדרה, לא סטייה |
| `kpi_items_currently_in_test` | 2 | **4** | אותו פיצול: `current_status = 1` בלבד מול `ms.is_active_work` = `testing`+`in_research` = 2+2 |
| `status_dist_percentage` | 0.3 / 2.4 / 96.3 | **8.7 / 65.2 / 17.4 / 8.7** | המכנה הישן הוא 630 — כל הפריטים, כולל 607 שסיימו. החדש הוא 23, האוכלוסייה הפעילה, ומסתכם ב־100.0 (§5.2). זה בדיוק "העיגול האפור של 96%" שהתוכנית חזתה |
| `completion_history_*` עם `shipmentId` | **HTTP 500** | סדרה אמיתית | `AND s.shipment_id = $n` על `shipments` שאין בה עמודה כזאת → 42703. נצפה ב־12 מקרים (6 חלונות × 2 scopes). בלי הפילטר הישן מחזיר `[]`, כי `shipment_snapshots_monthly` ריקה — כמו **כל 23 טבלאות ה־snapshot** |
| `kpi_avg_*` עם `workerId` | אפסים, HTTP 200, ובלי השדות `avgQueueSeconds` / `avgProcessingSeconds` / `treatedCount` | ערכים אמיתיים | 42703 על `worker_id`; ה־catch מחזיר `defaultKpis`, שהוא גם **חסר שלושה שדות** שהחוזה מבטיח. 12 מקרים |
| `customer_items_in_routes_percentage` | 100 קבוע | ערך אמיתי (100.0 בדאטהסט הזה, כי הכול מנותב) | ה־`LEFT JOIN` שהתנוון ל־`INNER`. ראו גם §9.4ב(1) — הגרסה הראשונה של הצד החדש נתנה 77–86 מסיבה אחרת לגמרי, ותוקנה |
| `station_total_processed_in_period` | 47 (תחנה 15, 30 יום) | **51** | 41 `result_submitted` + 10 `sent_to_research`; חמש נטישות reaper (`released_by_user` / `released_stale`) מוחרגות לפי §5.0(2). **הסתייגות:** החצי של "תחנות מחקר דיווחו 3 מתוך 30" **לא משוחזר** כאן — תחנות 12/13 מראות 9 ו־3 בשני הצדדים, כי ה־seeder כותב שורת `item_route_history` גם לצעדי מחקר. וגם: **אין שורות תחנה־0** בדאטהסט (0 שורות עם `test_station_id = 0`), כך שחצי ה"רעל" של הכניסה הזאת לא נבדק |

#### הפסיקות — הגדרה חדשה, שני הצדדים מוצדקים בהגדרתם

| מטריקה | ישן | חדש | הפסיקה |
|---|---|---|---|
| `station_items_in_queue` | 5 ו־3 לשתי התחנות הוויזואליות | **8 ו־8** | **הכיוון בתיעוד היה הפוך ותוקן.** הישן *פיצל* את התור לפי `item_routes.test_station_id`; החדש מדווח את תור **הסוג** על כל שורה. ה־ledger לא יכול אחרת: ל־interval במצב `queued` אין `station_id` כלל (0 מתוך 3,151 שורות; `metric_state.at_station = false`), רק `station_type_id`. המסקנה התפעולית: **אסור לסכם את העמודה הזאת** — שתי תחנות מאותו סוג מציגות את אותם 8. שלב 6 משנה את שם העמודה |
| `station_items_in_queue` / `_in_test` בתחנת מחקר | 2 ו־2 (תחנה 12) | **0 ו־0** | הישן השתמש בסטים הרחבים (`IN (2,4)`, `IN (1,5)`). Q3 מפריד: `queued_research` הוא **מאגר בלי תחנה** (64 שורות, כולן בלי `station_type_id`) ומדווח פעם אחת ב־`research_pool_queue`, ו־`in_research` מדווח ב־`_new_itemsInResearch`. **עד שלב 6 שתי העמודות הגלויות מציגות 0 למעבדות המחקר** — מוסכם, ומצוין כאן במפורש כדי שאיש לא יגלה בישיבה |
| `status_dist_count` עם `status=finished` | ספירת גמורים | **`[]`** | Q1 היא אוכלוסייה פעילה בלבד (§5.2), ו־`done` הוא המצב הסופי היחיד — סינון אליו מחזיר קבוצה ריקה. המעטפת נשמרת (מערך ריק, HTTP 200, אותם headers). הספירה המצטברת חיה ב־`finishedCumulative` של ה־sibling |
| `status_dist_history.count` (`done`) | 99 (תחת פילטר תחנה) | **607** | §5.3ב: "הושלמו" הוא סכימה רצה של סגירות `route_run` — 607 = כל הסגירות עד היום = `count(*) FROM route_run WHERE closed_at IS NOT NULL` = `count(*) FROM item_routes WHERE current_status = 3`. ל־`route_run` אין ממד תחנה/סוג/עובד/מצב, ולכן פילטר תחנה מצמצם את קווי ה־PIT ולא את הקו המצטבר — נאמר פר־מטריקה ב־`X-Metrics-Ignored-Filters: finishedCumulative:stationId` |
| `status_dist_history.percentage` | 0.3 / 2.4 / 96.3 | 8.7 / 65.2, ו־**null** על הקו המצטבר | אותו מכנה כמו למעלה; לסכום רץ אין נתח מתוך snapshot |
| `avg_times_is_today` | היום הקודם | היום העסקי הנכון | §7.2 — הישן ממקם את הדגל עם `setHours(0,0,0,0).toISOString()`, שהוא חצות **מקומית** מוצגת כיום UTC |
| `shipment_items_in_routes_percentage` | 58 / 175 / 66 | 31.6 / **125** / 34.5 | `coverage_pct = routed_units / shipments.amount`. **חורג מ־100% בדין** — בשמונה משלוחים לפחות, עד 450% (‏`amount = 2`, תשע יחידות נותבו בפועל). זו בדיוק הסיבה ש־§5.8 אוסר על `amount` לשמש מכנה של השלמה. אם המספר הזה אמור להיות ≤100 בפרודקשן, הבעיה היא ב־`shipments.amount`, לא בשאילתה |
| `item_type_items_in_routes_percentage` | 28 / 22 / 0 | **100.0 בכל סוג שיש לו פריטים** | מונה = `count(DISTINCT route_run_id)`, מכנה = מספר הפריטים מהסוג. אלה **אותן יחידות** (אביזר נושא סוג משלו ומקבל run משלו), ובדאטהסט הזה לכל פריט יש בדיוק run אחד (0 פריטים עם יותר מ־run אחד). לא באג — פשוט אין מה לגלות בדאטהסט שבו הכול נותב |
| `is_sent IS NOT TRUE` מול `= false` | — | — | **לא נבדק אמפירית:** ל־77 המשלוחים בדאטהסט אין ולו `is_sent` אחד שהוא NULL (77 `false`, 0 `true`, 0 NULL). הסמנטיקה אומתה ישירות ב־SQL: עבור NULL, `is_sent = false` מחזיר NULL (השורה נופלת) ו־`is_sent IS NOT TRUE` מחזיר true (השורה נשארת). ההבדל אמיתי; הדאטהסט פשוט לא מפעיל אותו |
| הפרש wall/work | — | — | **0 הפרות ב־924 בקשות** (7 endpoints × 6 חלונות × 11 פילטרים × 2 scopes), כולל בדיקת NULL־desync: אף `*Work*` לא עלה על התאום `*Wall*` שלו, ואף זוג לא הכיל NULL בצד אחד בלבד. ‏**2026-08-28 הוא יום שישי**, ‏0 שורות `work_span`, ולכן כל `work_*` של חלון "today" הוא 0 ו־`_new_offhoursMinutes` שווה לכל זמן הקיר — התנהגות נכונה של §2.8, לא אפס שקט |
| מעטפות ריקות | — | — | כל צירוף פילטר שאינו תואם דבר מחזיר **`[]` עם HTTP 200** ואותם headers. 52 מקרים כאלה במטריצה (בעיקר `status=finished` על Q4/Q6, ו־`itemSerial` בחלונות קצרים). אף 500, אף `null`, אף אובייקט שגיאה |
| החרגת אביזרים ממשכים (§5.0(8)) | — | — | הכלל אכוף, אבל הדאטהסט **לא מפעיל** את הכשל שבגללו נכתב: ב־seed לאביזרים יש משכי בדיקה אמיתיים (‏~14.6 דקות בממוצע בתחנת הקליטה), ולא הזוג הסינתטי בן ~0 של §4.7. ההשפעה הנמדדת היא 15.09→15.32 דקות בתחנה 14 (‏+1.5%), ולא הפער הגדול שהתוכנית חוזה בפרודקשן |

#### 9.4ב תיקונים שנעשו בצד החדש בעקבות הרתמה — באגים, לא הבדלים מכוונים

1. **`coverage_pct` של לקוח חילק יחידות בפריטים.** המונה הוא `count(DISTINCT rr.unit_id)` והמכנה היה `count(*) FROM items` — ויחידה היא **פריט־אב** (‏`route_run.unit_id = COALESCE(parent_item_id, item_id)`, אומת: 0 חריגות מ־630). התוצאה: כל לקוח קיבל כיסוי 77–86% למרות שכל יחידותיו נותבו (לקוח 5: ‏128/166 = 77.1% במקום 128/128 = 100%). ההטיה בדיוק בגודל שיעור האביזרים (111/630). המכנה — ו־`declared_amount` יחד איתו — סופר עכשיו יחידות. `parentsOnly` הוסר מהפרדיקט הזה: צמצום לאבות **הוא** ההגדרה של יחידה, לא אופציה.
2. **`busiestStation*` היה לא־דטרמיניסטי ביום לא־עבודה.** הדירוג הוא `work_hours DESC` (§5.10); בשישי כל ה־`work_seconds` הם 0, כל התחנות שוות, וה־sort החזיר את הראשונה שיצאה מהשאילתה: הכרטיס הכריז על "תפקודית 1" (13 צעדים, 0.0002 שעות קיר) בעוד "ויזואלית 1" עשתה 22 צעדים ו־0.0044. נוסף שובר־שוויון `work → wall → steps_processed → station_id`; שעון העבודה נשאר הקובע כשיש לו מה לומר.
3. **`kpi_busiest_station_wait_seconds` היה סכום כלל־מעבדתי תחת שם פר־תחנה.** נמדד ל־30 יום: 5,393,267 שניות על פני כל הסוגים, מול 268,273 שהישן דיווח לתחנה העמוסה. עכשיו הוא ההמתנה של **סוג התחנה העמוסה** (‏§5.4: התור שייך לסוג), והסכום הכלל־מעבדתי נשלח לצדו בשמו שלו — `_new_labWaitWorkSeconds` / `_new_labWaitWallSeconds`. כדי לאפשר זאת, ממד ה־`station` של Q4 נושא עכשיו גם את `station_type_id` (בטוח: 0 תחנות שה־intervals שלהן נושאים יותר מסוג אחד). **הערה:** לתחנת מחקר המספר הזה הוא 0 בדין — ל־`queued_research` אין `station_type_id` כלל, וזמן ההמתנה למחקר מדווח בנפרד ב־`_new_researchWait{Wall,Work}Minutes`.
4. **Q2א תוכננה כ־hash join ו־`@>` התנוונה ל־Join Filter** — ראו §9.4ג.
5. **תיעוד:** ההערה בראש `tests/by-station/route.ts` טענה שהישן "כפל את התור על פני כל תחנות הסוג". ההפך הוא הנכון (הישן פיצל, החדש חוזר). תוקן, עם המספרים.

#### 9.4ג ביצועים — נמדד, §6.4

זמני תגובה מקצה לקצה, שרת `next dev` (לא build פרודקשן), p95 על 22 מקרים לכל endpoint **בתקרת 13 החודשים**:

| endpoint | p50 | p95 | סף §6.4 |
|---|---|---|---|
| `tests/customer-performance` | 981 | **1,373** | 2,000 |
| `tests/item-types` | 853 | **1,317** | 2,000 |
| `tests/shipments` | 415 | 510 | 2,000 |
| `tests/by-station` | 222 | 274 | |
| `stats/completion-history` | 183 | 230 | |
| `tests/kpis` | 119 | 174 | |
| `tests/status-distribution/history` | 157 | 172 | |
| `tests/average-times` | 129 | 144 | |
| `tests/slow-items` | 113 | 135 | |
| `tests/status-distribution` | 75 | 108 | |

אף endpoint אינו חוצה את סף שתי השניות. זמני ה־SQL בלבד (‏`EXPLAIN (ANALYZE, BUFFERS)` על הטקסט שמשרת את הבקשות, דרך `QUERY_BUILDERS`): Q7 לקוח 1,053ms · Q7 סוג פריט 988ms · Q7 משלוח 280ms · Q3 86ms · Q4 יומי 13 חודשים 51ms · Q6 46ms · Q5 36ms · **Q2א 36ms** · Q4 תחנה 17ms · Q2ב 6ms · Q1 3ms.

**הממצא: Q2א תוכננה כ־hash join, ותוקנה מבנית.** בטקסט של §5.3 (‏`LEFT JOIN` רגיל) ועם `scope = open_shipments`, ה־planner הופך את ה־EXISTS של המשלוחים ל־hashed SubPlan על סריקת ה־intervals, מממש את כל 5,819 השורות הלא־סופיות, ואז מוריד את `valid_range @> g.at_ts` מ־Index Cond ל־**Join Filter**: `Rows Removed by Join Filter: 2,300,540`, ‏**1,731ms**. עם `scope = all` אותו טקסט בדיוק מקבל Index Scan על `isi_range_live` ורץ 79ms. זו מחלקת הכשל של §5.0(9) שנכנסת מדלת ה־planner במקום מדלת הפרדיקט, והעלות שלה היא (שורות לא־סופיות × ימי גריד) — כלומר ~1.2 מיליארד הערכות בשנה 3 במקום 2.3 מיליון. Q2א נכתבה מחדש כ־`LEFT JOIN LATERAL` עם aggregate (שאינו ניתן לשיטוח), כך ש**probe אחד ליום גריד** הוא מבני ולא תלוי־planner: **36ms**, פי 48 מהר יותר, ו־`status-distribution/history` ירד מ־442ms ל־273–333ms. השקילות אומתה שורה־בשורה על 96 צירופי (חלון × פילטר × scope): **0 הפרשים.**

**ההסלמה: Q7 אינה חסומה בתקרת 13 החודשים.** §5.8 מגדיר את התקדמות הישות כשאילתת מצב בלי חלון, ולכן היא סורקת את **כל** ה־`route_run` שנוצרו אי־פעם. אחד ממרכיבי העלות שלה הוא `work_seconds_between(rr.opened_at, rr.closed_at)` — נמדד **0.53ms לקריאה** (‏321ms ל־607 ריצות סגורות, בבידוד). זה לינארי במספר ה־runs לכל אורך החיים: לפי קצב §6.1 (‏~75k סגירות לשנה) הזוג הזה לבדו הוא ~40 שניות לכל שנת היסטוריה. כבר היום Q7 היא 1.0–1.1 שניות והיא ה־endpoint האיטי ביותר. **דרושה החלטה לפני פרודקשן.** האפשרויות: (א) לאחסן `work_seconds` על `route_run` בזמן הסגירה (‏§2.7 אוסר לאחסן *יחס*, לא משך); (ב) לגזור את משך העבודה של ה־run מסכום ה־`work_seconds` של ה־intervals שלו (הם מרצפים את הריצה ברציפות מכוח ה־EXCLUDE); (ג) להחיל חלון גם על Q7, ולקבל שהמסך יענה "מאז X".

#### 9.4ד סבב אימות שני — מה נמצא עוד פתוח, ומה תוקן

הסבב הזה בדק את תיקוני הסקירה האדוורסרית מול ה־DB, לא מול הדוחות. שישה מהם עמדו במלואם; ארבעה דברים היו עדיין פתוחים ותוקנו כאן, כולם נמדדו.

| מה נמצא | הראיה | התיקון |
|---|---|---|
| **`npx tsc --noEmit` לא היה נקי.** `_new_unitsProcessed` הוצהר `number` ב־`by-station` ומופה מ־`w.units_processed`, שהוא `undefined` תחת ממד `station` — בדיוק הממד ש־§5.0(6) אוסר עליו `unit_id` כמפתח דה־דופליקציה, ולכן `buildFlow` לא פולט את העמודה כלל | ‏TS2322 יחיד; ובפועל `JSON.stringify` השמיט את המפתח, כך שהחוזה הבטיח שדה שהתשובה מעולם לא נשאה (‏0 מופעים ב־600 בקשות סריקה) | השדה הוסר מה־interface ומהמיפוי, עם הערה למה הוא לא יכול להיות שם |
| **Q3 (לוח התחנות) נשא את אותו כשל JIT שתוקן ב־Q7.** שלוש תת־שאילתות מתואמות עם אותו `WHERE` בדיוק, שנבדלות רק ב־aggregate | עלות מוערכת **358,562** מול `jit_above_cost = 100,000`; ‏`JIT: Functions 65`, ‏54–139ms מתוך statement של 73–167ms. Q3 רצה בכל בקשת `by-station` וגם במסך ההגדרות | ‏CTE אחד מקובץ לפי `station_type_id` + CTE אחד למאגר המחקר: עלות **1,696**, **אין בלוק JIT**, ‏**9.8–16.5ms**. שקילות: 10 שורות, 120 תאים, **0 הפרשים** (שני הטקסטים באותה טרנזקציה, כדי ש־`now()` יהיה זהה) |
| **Q8 (איכות) נשאה אותו כשל, חמור יותר.** ארבע תת־שאילתות מתואמות על אותה קבוצת `route_run_id = ANY(array_agg(DISTINCT …))`, שנבדלות רק ב־`state_key` ובשעון | עלות **699,200** — מעל `jit_above_cost` **וגם** מעל `jit_optimize_above_cost = 500,000`, ולכן הידור עם inlining ו־optimization: ‏**1,007–1,203ms של LLVM** בתוך statement של 1,048–1,244ms, שהתוצר שלו 844 שורות | אותה קבוצה, כתובה כ־join במקום כמערך: `base` → `DISTINCT (יום, סוג, run)` → join ל־intervals של המחקר → aggregate אחד לקבוצה. עלות **8,549**, **אין בלוק JIT**, ‏**24–46ms** (פי ~35). שקילות: **44 מקרים** (4 חלונות × 11 פילטרים), 6,945 שורות, **83,340 תאים, 0 הפרשים** |
| **`?? 0` שרד בשלושה מסכים.** `itemsInRoutesPercentage` הוחזר כ־0 כאשר Q7 עונה NULL — מכנה 0, כלומר ישות שלא הצהירה על פריטים | חי בדאטהסט: **2 מתוך 6 לקוחות ו־3 מתוך 11 סוגי פריט** בלי פריט אחד. הלקוח "מנדי" קיבל `successPercentage: null` לצד `itemsInRoutesPercentage: 0` — אותה שורה, שתי תשובות סותרות לאותה שאלה | השדה הורחב ל־`number \| null` בשלושת ה־routes |
| **החוזה על החוט לא עודכן כשהורחבו האחוזים.** `successPercentage` / `completionPercentage` כבר החזירו NULL, אבל `src/types/dashboard.ts` עדיין הצהיר `number` — ולכן הטבלאות רינדרו את **המחרוזת `"null%"`** על המסך | ‏`{row.completionPercentage}%` בתוך template literal; ‏`<ProgressCell pct={null}>` נותן `width: "null%"`, כלומר פס ריק בלי הודעה | שלושת השדות הורחבו ב־`types/dashboard.ts`, ‏`ProgressCell` מקבל `number \| null`, ושלוש הטבלאות מציגות `-` — אותה מוסכמה שכבר נהוגה שם ל־duration חסר |
| **חבילת ה־observability הייתה מהבהבת — 2 כשלים ב־8 הרצות מלאות.** `node --test` מריץ קבצים במקביל; כותרת החבילה חשבה על אי־truncate של ה־**ledger** ופספסה את הטבלה המשותפת השנייה: שתי חבילות ה־ledger בונות מחדש גם את `work_calendar_version`, ובזמן הבנייה אין שורת `is_current` | ‏`/api/cron/metrics-selfcheck` קורא `calendar_horizon_days` ראשון ומתייחס ל־NULL כ"רפא עכשיו", ה־self-heal של §7.4 נדלק, וה־dynamic import של `@hebcal/core` (ESM בלבד) מת תחת ה־runner ב־CJS עם `ERR_PACKAGE_PATH_NOT_EXPORTED` — HTTP 500 במקום 200. אותה שגיאה בשני הכשלים | הבדיקה תופסת עכשיו את **אותו** advisory lock (‏526050825) ששתי החבילות האחרות מחזיקות, למשך הבדיקה הזאת בלבד: **6/6 הרצות ירוקות, 0 שגיאות hebcal**. **הפער נשאר:** ענף ה־self-heal עצמו עדיין לא ניתן להרצה מתוך ה־runner, ולכן אינו מכוסה |
| **רתמת ה־parity הייתה RED על החשבונאות של עצמה.** `expandStatusHistory` כתבה `finishedCumulative` / `_new_finishedToday` / `_new_activeTotal` לשורת־היום של **שני** הצדדים; ‏`compareRow` מזהה "שדה חדש בלבד" לפי **נוכחות המפתח**, ולכן שלושה שדות שלישן מעולם לא היו נספרו כ־`DIFFERS` | ‏3 הפרשים בלתי מוסברים, כולם בצורה `old=- new=<ערך>` | המפתחות נכתבים רק כשהצד באמת נושא אותם |

**המצב אחרי התיקונים.** ‏`tsc --noEmit` נקי · `next build` עובר · `npm test` 50/50 · עם `TEST_DATABASE_URL` (‏+`TEST_APP_URL`+`AUTH_SECRET`) 108/108 · ‏`eslint --max-warnings=0` נקי · `metrics_selfcheck()` נקייה · **PARITY: GREEN**, 10 endpoints × 132 מקרים, 0 הפרשים בלתי מוסברים, 0 שגיאות.

**הרתמה מדרגת עכשיו את שדות ה־`_new_` — הוכח בשבירה מכוונת, אחת בכל פעם, ואז שחזור:** ‏`_new_typeWaitWorkHours` שאולץ ל־null בכל מקרה ⇒ `N NEW_ALWAYS_NULL`, PARITY RED · חצי־העבודה של הזוג ששונה שמו ⇒ `P NEW_UNPAIRED`, RED · שני השעונים שהוחלפו ⇒ `C NEW_CLOCKS_CROSSED`, RED · שחזור ⇒ GREEN.

**מה שנשאר פתוח (החלטת תכנון, לא תעתיק).** התרשים ב־`StationLoadTable.tsx` מצייר עמודת `itemsInQueue` לכל תחנה, וזה מספר ברמת **הסוג** שחוזר על עצמו — שלוש התחנות התפקודיות מציגות 4 כל אחת עבור תור אחד של 4, והעין מסכמת ל־12. העובדה מתועדת בשלושה מקומות (כותרת `by-station/route.ts`, ‏`station-counters.ts`, §9.4א) אבל **לא בקומפוננטה עצמה**, שהיא המקום שבו הטעות תיעשה. שלב 6 משנה את שם העמודה; עד אז זה ידוע ומוסכם.
