@echo off
REM ===========================================================================
REM Release stale tests - runs every 5 minutes via Task Scheduler.
...
curl --fail --silent --show-error -X POST -H "x-cron-secret: %CRON_SECRET%" http://localhost/api/cron/release-stale-tests
if errorlevel 1 (
