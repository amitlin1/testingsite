  ...
  try {
    ...
    const rows = await prisma.$queryRawUnsafe<any[]>(`
      ...
      SELECT
        ...
        wr.worker_name as recieving_worker_name,
        ...
        src.source_desc,
        s.sending_worker_id,
        sw.worker_name as sending_worker_name,
        s.poc_details,
      ...
      LEFT JOIN workers wr ON s.recieving_worker_id = wr.worker_id
      LEFT JOIN sources src ON s.source_id = src.source_id
      LEFT JOIN workers sw ON s.sending_worker_id = sw.worker_id
    ...
    const newShipment = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.shipments.create({
        data: {
          ...
          makat: makat || 0,
          amount,
          recieving_worker_id: recieving_worker_id || null,
          signature_path: null,
