...
export default function ShipmentHistoryDialog({
  open,
  onClose,
  shipment,
  ...
  const fetchHistory = React.useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // NO ZERO-DAY FILTER. It used to drop every day on which this shipment had
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            <Typography variant="h6">היסטוריית משלוח: {shipment.shipmentCode}</Typography>
      ...
      <DialogContent sx={{ flex: 1, p: 3, display: "flex", flexDirection: "column" }}>
        {loading ? (
          ...
          <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", flex: 1 }}>
            <Typography color="text.secondary">אין נתונים היסטוריים למשלוח זה</Typography>
          </Box>
        ) : (
