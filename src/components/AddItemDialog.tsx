...
import { X, ScanLine, Plus, Trash2, Package } from "lucide-react";
...
/* =========================================================================
   ...
   Fields use the unified <SearchableCombobox> (searchable) per the app-wide
   "one combobox" decision. The container may seed the internal form via the
   `initialForm` prop (+ a remount key) — used for QR autofill, shipment
   auto-fill, and batch continuation, which write into the form from outside.
   ========================================================================= */
...
type Props = {
  ...
  /** Seed the internal form on mount / open. Used by the container to write
      into the form from outside (QR autofill, shipment auto-fill, batch
      continuation). Pair with a changing React `key` to re-seed. */
  initialForm?: Partial<NewItemForm>;
};
...
export default function AddItemDialog(props: Props) {
  ...
  React.useEffect(() => { if (open) { setForm({ ...empty, ...initialForm }); setSubs([]); } }, [open]); // eslint-disable-line react-hooks/exhaustive-deps
  React.useEffect(() => { props.onFormChange?.(form); }, [form]); // eslint-disable-line react-hooks/exhaustive-deps
  if (!open) return null;
                    ...
                    {shipmentItems.map((it) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => { setForm((f) => ({ ...f, itemType: String(it.item_type_id), ...(it.makat != null ? { makat: String(it.makat) } : null) })); }}
                        >
...
/* Searchable select — the unified <SearchableCombobox>, adapted to the simple
   string value / {value,label} option API this dialog uses. */
