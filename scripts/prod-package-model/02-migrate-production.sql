-- =============================================================================
-- 02 — מיגרציית מודל המארזים על DB הייצור
-- =============================================================================
-- זה בדיוק prisma/migrations/20260915120000_package_model/migration.sql, עטוף
-- בטרנזקציה אחת ועם רישום ב-_prisma_migrations, כך ש-`prisma migrate deploy`
-- של הגרסה החדשה יזהה שהיא כבר הוחלה ולא יריץ אותה שוב.
--
-- לפני שמריצים:
--   1. לעצור את האפליקציה (הקונטיינר של next-app) — אסור שהגרסה הישנה תכתוב
--      בזמן שהעמודות משתנות.
--   2. גיבוי:  pg_dump -U <user> -Fc <db> > before-packages.dump
--   3. להריץ את 01-diagnose-production.sql ולוודא שסעיף 0 מראה
--      items.parent_item_id = true ו-items.package_id = false.
--
-- הרצה ב-DBeaver: כל הקובץ עם Alt+X. כל שגיאה מבטלת הכול (טרנזקציה אחת);
-- אם נעצר באמצע, להריץ ROLLBACK; ולדווח.
-- =============================================================================

BEGIN;

-- בדיקות מקדימות: הסכימה ישנה ומיגרציה זו לא הוחלה עדיין
DO $pre$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'items' AND column_name = 'parent_item_id') THEN
    RAISE EXCEPTION 'items.parent_item_id לא קיימת — או שהמיגרציה כבר הוחלה, או שזו לא הסכימה הצפויה. לא בוצע כלום.';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM item_status WHERE item_status_id = 6 AND TRIM(item_status_desc) <> 'ממתין לפריטי המארז') THEN
    RAISE EXCEPTION 'סטטוס 6 כבר תפוס בשם אחר (%). יש לפנות אותו לפני המיגרציה.', (SELECT item_status_desc FROM item_status WHERE item_status_id = 6);
  END IF;
END
$pre$;

-- ---- prisma/migrations/20260915120000_package_model/migration.sql (verbatim) ----
-- ===========================================================================
...
-- רישום המיגרציה כפי ש-prisma migrate deploy היה רושם (ה-checksum הוא
-- sha256 של קובץ המיגרציה, ולכן חייב להתאים לקובץ שבגרסה שתיפרס).
