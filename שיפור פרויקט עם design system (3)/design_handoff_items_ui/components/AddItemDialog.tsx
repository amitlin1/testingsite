...
export default function AddItemDialog(props: Props) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div onClick={stop} dir="rtl" className="sh-scroll" style={{ width: "min(680px,94vw)", maxHeight: "88vh", overflow: "hidden", display: "flex", flexDirection: "column", background: "#fff", borderRadius: 18, boxShadow: "rgba(0,0,0,0.22) 3px 5px 30px 0", fontFamily: "var(--sh-font)", color: "#1d1d1f" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, padding: "20px 24px", borderBottom: "1px solid #e0e0e0" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {onScan && (
              <button onClick={onScan} title="סרוק ברקוד" className="sh-btn sh-btn-secondary" style={{ padding: "8px 14px", fontSize: 14 }}>
        ...
        {/* ---- body ---- */}
        <div className="sh-scroll" style={{ padding: 24, overflowY: "auto" }}>
          {/* shipment (full width) */}
          <Field label="משלוח">
            ...
            <div style={{ marginTop: 16, border: "1px solid #e0e0e0", borderRadius: 12, overflow: "hidden" }}>
              ...
              {loadingShipmentItems ? (
                <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 10 }}>
                  <div style={{ height: 38, borderRadius: 8, background: "linear-gradient(90deg,#eee,#f5f5f7,#eee)", backgroundSize: "200% 100%", animation: "sh-shimmer 1.4s infinite" }} />
                  <div style={{ height: 38, borderRadius: 8, background: "linear-gradient(90deg,#eee,#f5f5f7,#eee)", backgroundSize: "200% 100%", animation: "sh-shimmer 1.4s infinite" }} />
                </div>
              ) : shipmentItems.length > 0 ? (
                <table className="sh-table">
                  <thead>
                    <tr>
                      <th className="sh-th">סוג פריט</th>
                      <th className="sh-th sh-th--center">מק״ט</th>
                      <th className="sh-th sh-th--center">כמות</th>
                      <th className="sh-th sh-th--center">מדגם</th>
                      <th className="sh-th" style={{ width: 150 }}>התקדמות</th>
                    </tr>
                  </thead>
                  <tbody>
                    {shipmentItems.map((it) => {
                      ...
                      return (
                        <tr
                          key={it.id}
                          className="sh-row sh-row--clickable"
                        ...
                        >
                          <td className="sh-td" style={{ fontWeight: 600 }}>{it.item_type_desc}</td>
                          <td className="sh-td" style={{ textAlign: "center", fontVariantNumeric: "tabular-nums" }}>{it.makat ?? "—"}</td>
                          <td className="sh-td" style={{ textAlign: "center", color: "#444", fontVariantNumeric: "tabular-nums" }}>{it.total_quantity}</td>
                          <td className="sh-td" style={{ textAlign: "center", fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{it.sample_count}</td>
                          <td className="sh-td">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 20 }}>
            ...
            <Field label="מס׳ סיריאלי"><input className="sh-input" value={form.serialNumber} onChange={(e) => set("serialNumber", e.target.value)} placeholder="לדוגמה: 88312" style={{ fontVariantNumeric: "tabular-nums" }} /></Field>
            <Field label="מק״ט"><input className="sh-input" value={form.makat} onChange={(e) => set("makat", e.target.value)} placeholder="לדוגמה: 4567" style={{ fontVariantNumeric: "tabular-nums" }} /></Field>
            <Field label="דגם"><input className="sh-input" value={form.model} onChange={(e) => set("model", e.target.value)} placeholder="תיאור הדגם" /></Field>
            <Field label="יצרן"><input className="sh-input" value={form.manufacturer} onChange={(e) => set("manufacturer", e.target.value)} placeholder="שם היצרן" /></Field>
            <Field label="מס׳ יצרן"><input className="sh-input" value={form.manufacturerNo} onChange={(e) => set("manufacturerNo", e.target.value)} placeholder="מס׳ יצרן" /></Field>
          </div>
            ...
            {subs.map((sub, i) => (
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12, padding: 14, border: "1px solid #e0e0e0", borderRadius: 12, background: "#fafafc" }}>
                <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  ...
                  <input className="sh-input" placeholder="מס׳ סיריאלי" value={sub.serialNumber} onChange={(e) => setSubs((s) => s.map((x, j) => j === i ? { ...x, serialNumber: e.target.value } : x))} />
                  <input className="sh-input" placeholder="מק״ט" value={sub.makat} onChange={(e) => setSubs((s) => s.map((x, j) => j === i ? { ...x, makat: e.target.value } : x))} />
                  <input className="sh-input" placeholder="דגם" value={sub.model} onChange={(e) => setSubs((s) => s.map((x, j) => j === i ? { ...x, model: e.target.value } : x))} />
                </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 24px", borderTop: "1px solid #e0e0e0", background: "#fafafc" }}>
          <div style={{ flex: 1 }} />
          <button className="sh-btn sh-btn-secondary" onClick={onClose}>ביטול</button>
          <button className="sh-btn sh-btn-primary" disabled={!valid} onClick={() => onSubmit(form, subs)}>
